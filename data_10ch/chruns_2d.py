...
if __name__ == "__main__":
    ...
    print("Complicated (whether to use dt prior or not) is set to: {}".format(args.complicated))
    print("ntotal: {}".format(args.ntotal))
    print("nrnd: {}".format(args.nrnd))
    trainsC = Trains(emg=args.emg)
    trains = trainsC.trains
    queriedchs, maxchs = run_ch_stats_exps(trains, args, repeat=args.repeat, complicated=args.complicated, ntotal=args.ntotal, nrnd=args.nrnd)
