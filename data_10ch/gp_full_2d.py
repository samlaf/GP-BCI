...
def run_ch_stats_exps(trainsC, emg=emg, dt=dt, uid='', repeat=25, continue_opt=True, k=2, dtprior=False, ntotal=100, nrnd = [15,76,10], sa=True, multkern=False, symkern=False, ARD=False, constrain=True):
    if uid == '':
        ...
        for i,n1 in enumerate(nrnd):
            ...
            modelsD = train_model_seq_2d(trainsC,n_random_pts=n1, n_total_pts=ntotal,
                                        ...
                                        dt=dt, emg=emg, sa=sa, multkern=multkern,
                                         symkern=symkern, constrain=constrain)
